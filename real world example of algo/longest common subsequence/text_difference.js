function textDiff(s1, s2, splitByWords) {
  ...
  return { lcs: lcs, deleted: result1.join(""), added: result2.join("") };
}
...
//A dynamic Programming approach to fnd lcs of s1 and s2
...
helper = (s1, s2, i1, i2, memo) => {
  ...
  if (s1[i1] == s2[i2]) {
    memo[i1][i2] = s1[i1] + helper(s1, s2, i1 + 1, i2, memo);
    return memo[i1][i2];
  }
  let result;
